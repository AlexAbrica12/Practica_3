...
static char ledcolor_data[10];
static int ledcolor_data_len = 0;

//static uint8_t steps_data;
static char steps_data[10];
static int steps;
static int steps_data_len = 0;
static int steps_lengt = 0;

//static uint8_t steps_data;
static char size_data[10];
static int size;
static int size_data_len = 0;
static int size_lengt = 0;

static char name_data[20];
static int name_data_len = 0;
...
#define DEFAULT_SHOELACE "untie"
#define DEFAULT_LEDCOLOR "000000"
                  ...
                  coap_pdu_t *response)
{
    ...
    if (strcmp (shoelace_data, DEFAULT_SHOELACE) == 0) {
        coap_pdu_set_code(response, COAP_RESPONSE_CODE_CREATED);
    } else {
        coap_pdu_set_code(response, COAP_RESPONSE_CODE_CHANGED);
    }
                  ...
                  coap_pdu_t *response)
{
    ...
    coap_add_data_large_response(resource, session, request, response,
                                 query, COAP_MEDIATYPE_TEXT_PLAIN, 60, 0,
                                 (size_t)ledcolor_data_len,
                                 (const u_char *)ledcolor_data,
                                 NULL, NULL);
    printf("Led color: %s\n", ledcolor_data);   
}
                  ...
                  coap_pdu_t *response)
{
    ...
    if (strcmp (ledcolor_data, DEFAULT_LEDCOLOR) == 0) {
        coap_pdu_set_code(response, COAP_RESPONSE_CODE_CREATED);
    } else {
        coap_pdu_set_code(response, COAP_RESPONSE_CODE_CHANGED);
    }
    ...
    if (size == 0) {      // re-init //
        snprintf(ledcolor_data, sizeof(ledcolor_data), DEFAULT_LEDCOLOR);
        ledcolor_data_len = strlen(ledcolor_data);
    } else {
        ledcolor_data_len = size > sizeof (ledcolor_data) ? sizeof (ledcolor_data) : size;
        memcpy (ledcolor_data, data, ledcolor_data_len);
    }
}
                  ...
                  coap_pdu_t *response)
{
    ...
    coap_add_data_large_response(resource, session, request, response,
                                 query, COAP_MEDIATYPE_TEXT_PLAIN, 60, 0,
                                 (size_t)steps_data_len,
                                 (const u_char *)steps_data, // steps_data to steps  <-----------
                                 NULL, NULL);
    //steps_lengt = sscanf(steps_data, "%d", &steps);
    printf("Steps: %s\n", steps_data);
}

static void
hnd_steps_put(coap_resource_t *resource,
                  coap_session_t *session,
                  const coap_pdu_t *request,
                  const coap_string_t *query,
                  coap_pdu_t *response)
{
    ...
    size_t total;
    const unsigned char *data;
    static unsigned char payload[20];

    coap_resource_notify_observers(resource, NULL);

    if (strcmp (steps_data, DEFAULT_STEPS) == 0) {
    //if (steps_data == DEFAULT_STEPS){
        coap_pdu_set_code(response, COAP_RESPONSE_CODE_CREATED);
    ...
    if (size == 0) {      // re-init //
        //steps_data = DEFAULT_STEPS;
        snprintf(steps_data, sizeof(steps_data), DEFAULT_STEPS);
        steps_data_len = strlen(steps_data);
        steps = 0;
        //steps_data_len = sizeof(steps_data);
    } else {
        steps_data_len = size > sizeof (steps_data) ? sizeof (steps_data) : size;
        memcpy (steps_data, data, steps_data_len);
        steps_lengt = sscanf(&data, "%d", &steps);
        //steps_data = (uint8_t)(payload);
                  ...
                  coap_pdu_t *response)
{
    ...
    coap_add_data_large_response(resource, session, request, response,
                                 query, COAP_MEDIATYPE_TEXT_PLAIN, 60, 0,
                                 (size_t)size_data_len,
                                 (const u_char *)size_data,  // size_data to size  <-----------
                                 NULL, NULL);
    //size_lengt = sscanf(size_data, "%d", &size);
    printf("Size: %d\n", size);                              
}
                  ...
                  coap_pdu_t *response)
{
    ...
    if (size == 0) {      // re-init //
        ...
        name_data_len = strlen(name_data);
    } else {
        name_data_len = size > sizeof (name_data) ? sizeof (name_data) : size;
        memcpy (name_data, data, name_data_len);
    }
}
...
static void coap_example_server(void *p)
{
    ...
    snprintf(size_data, sizeof(size_data), DEFAULT_SIZE);
    size_data_len = strlen(size_data);
    //size_data_len = sizeof(size_data);
    size = 20;
    ...
    while (1) {
        ...
        coap_register_handler(resource, COAP_REQUEST_GET, hnd_ledcolor_get);
        coap_register_handler(resource, COAP_REQUEST_PUT, hnd_ledcolor_put);
        coap_register_handler(resource, COAP_REQUEST_DELETE, hnd_ledcolor_delete);
        // We possibly want to Observe the GETs //
        coap_resource_set_get_observable(resource, 1);
        coap_add_resource(ctx, resource); 
        ...
        coap_register_handler(resource, COAP_REQUEST_GET, hnd_steps_get);
        coap_register_handler(resource, COAP_REQUEST_PUT, hnd_steps_put);
        coap_register_handler(resource, COAP_REQUEST_DELETE, hnd_steps_delete);
        // We possibly want to Observe the GETs //
        coap_resource_set_get_observable(resource, 1);
        coap_add_resource(ctx, resource);  
        ...
        coap_register_handler(resource, COAP_REQUEST_GET, hnd_size_get);
        // We possibly want to Observe the GETs //
        coap_resource_set_get_observable(resource, 1);
        coap_add_resource(ctx, resource);        
        ...
        coap_register_handler(resource, COAP_REQUEST_GET, hnd_name_get);
        coap_register_handler(resource, COAP_REQUEST_PUT, hnd_name_put);
        coap_register_handler(resource, COAP_REQUEST_DELETE, hnd_name_delete);
        // We possibly want to Observe the GETs //
        coap_resource_set_get_observable(resource, 1);
        coap_add_resource(ctx, resource);        
